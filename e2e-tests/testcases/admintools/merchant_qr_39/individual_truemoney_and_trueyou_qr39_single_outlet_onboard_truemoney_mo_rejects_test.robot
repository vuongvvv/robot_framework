...
*** Test Cases ***
TC_O2O_27455
    [Documentation]    [O2O Merchant Onboard - Tag39] Individual - TrueMoney  and TrueYou - qr39 merchant got rejected by TrueMoney MO
    ...
    Verify Contact Information    address=-${SPACE}    sub_district=-${SPACE}    district=-${SPACE}    city_province=-${SPACE}    post_code=-${SPACE}
    Verify Billing Address    address=-${SPACE}    sub_district=-${SPACE}    district=-${SPACE}    city_province=-${SPACE}    post_code=-${SPACE}
    ...
    Verify Shop Address    road=${EMPTY}
    Clean Environment
    ...
    Open Browser With Option    ${TRUE_MONEY_ADMIN_PORTAL_URL}
    Login TrueMoney Admin Portal    ${TRUE_MONEY_ADMIN_PORTAL_USERNAME}    ${TRUE_MONEY_ADMIN_PORTAL_PASSWORD}
    Update TrueMoney Merchant Operation Status    ${TRUEMONEY_BRAND_ID}    CERTIFICATED_FAILED    Automation Failed ระบบอัตโนมัติล้มเหลว
    
    Wait Data To Sync On Cms    ${TEST_DATA_CMS_PROJECT_ID}    shop-trueyou    ${SHOP_TRUEYOU_CONTENT_ID}    data.tmnShopStatus    REJECT
    Get Content    ${TEST_DATA_CMS_PROJECT_ID}    brand-callverification    ${BRAND_CALL_VERIFICATION_CONTENT_ID}
    Response Property Should Be Equal As String    data.callVerStatus    PASSED
    Get Content    ${TEST_DATA_CMS_PROJECT_ID}    shop-callverification    ${SHOP_CALL_VERIFICATION_CONTENT_ID}
    Response Property Should Be Equal As String    data.callVerStatus    PASSED
    Get Content    ${TEST_DATA_CMS_PROJECT_ID}    brand-truemoney    ${BRAND_TRUEMONEY_CONTENT_ID}
    Response Property Should Be Equal As String    data.status    REJECT
    Response Property Should Be Equal As String    data.fraudStatus    PASSED
    Response Property Should Be Equal As String    data.tmnApprovalStatusReason    Automation Failed ระบบอัตโนมัติล้มเหลว
    Get Content    ${TEST_DATA_CMS_PROJECT_ID}    shop-truemoney    ${SHOP_TRUEMONEY_CONTENT_ID}
    Response Property Should Be Equal As String    data.status    REJECT
    Get Content    ${TEST_DATA_CMS_PROJECT_ID}    shop-contract-truemoney    ${SHOP_CONTRACT_TRUEMONEY_CONTENT_ID}
    Response Property Should Be Equal As String    data.status    WAITING_REVIEW
    ...
    Get Content    ${TEST_DATA_CMS_PROJECT_ID}    shop-trueyou    ${SHOP_TRUEYOU_CONTENT_ID}
    Response Property Should Be Equal As String    data.status    WAITING_REVIEW
    Response Property Should Be Equal As String    data.tmnShopStatus    REJECT
    Get Content    ${TEST_DATA_CMS_PROJECT_ID}    terminal-truemoney    ${TERMINAL_TRUEMONEY_REF_ID}
    Response Property Should Be Equal As String    data.status    REJECT
    
    Reject Shop Contract Approval    ${TRUEMONEY_SHOP_ID}
    Clean Environment
    
    Wait Data To Sync On Cms    ${TEST_DATA_CMS_PROJECT_ID}    shop-truemoney    ${SHOP_TRUEMONEY_CONTENT_ID}    data.status    REJECT
    Get Content    ${TEST_DATA_CMS_PROJECT_ID}    brand-callverification    ${BRAND_CALL_VERIFICATION_CONTENT_ID}
    Response Property Should Be Equal As String    data.callVerStatus    PASSED
    Get Content    ${TEST_DATA_CMS_PROJECT_ID}    shop-callverification    ${SHOP_CALL_VERIFICATION_CONTENT_ID}
    Response Property Should Be Equal As String    data.callVerStatus    PASSED
    ...
    # WHEN TrueMoney Admin approves merchant registration on TrueMoney Admin Portal (aka TrueMoney MO)
    # THEN Ascend Commerce platform should receive this status update
    # When TrueMoney Admin approves shop contract on TrueMoney Admin Portal (aka TrueMoney MO)
    # Then Ascend Commerce platform should receive this status update
    
    # https://truemoney.atlassian.net/browse/ASCO2O-24526
    # Scenario: Merchant TMN + TY not approved from their side
    # given: Register new merchant tag39 with provider TMN + TY and not approved on their side
    
    # when: TMN or TY was rejected the merchant
    
    # then: Should not create merchant tag29 on merchant-transaction and merchant tag39 unable to have ability same as merchant tag29
    # and merchant data are not created in MTransaction DB
    # and rpp-address is not created
    # and rpp-previlege data is not created
    # and rpp-location data is not created
    # and rpp-merchant data is not created